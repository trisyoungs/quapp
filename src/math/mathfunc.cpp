...
// Error Function
double QuappMath::erfc(double x)
{
...
double QuappMath::erf(double x)
{
	return (1.0 - erfc(x));
}

// Random Number Generator (0 - 1)
double QuappMath::random()
{
...
int QuappMath::randomi(int range)
{
	// Returns a random number from 0->(range-1) inclusive.
...
int QuappMath::power(int i, int p)
{
	static int result, n;
	result = i;
	if (p == 0) result = 1;
...
// Sign function
int QuappMath::sgn(int x)
{
	return (x < 0 ? -1 : x > 0);
}

// Calculate area of triangle from supplied coordinates
double QuappMath::triangleArea(double ax, double ay, double bx, double by, double cx, double cy)
{
