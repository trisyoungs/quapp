...
template<class T> class Simplex
{
	...
	*/
	bool accept(double trialCost, int vertexId, double temperature)
	{
		...
		else
		{
			...
			if (QuappMath::random() < exp(-deltaCost/temperature)) return true;
		}
		return false;
	}
	...
	*/
	void findExtremes(double temperature)
	{
		...
		do
		{
			n = int(QuappMath::random()*nVertices_);
		} while (n == vWorst_);
		if (QuappMath::random() < exp(-fabs(costs_[vBest_]-costs_[n])/temperature))
		{
		...
		n = int(QuappMath::random()*nVertices_);
		if (QuappMath::random() < exp(-fabs(costs_[vWorst_]-costs_[n])/temperature))
		{
			// Swap points if necessary
		...
		if (QuappMath::random() < exp(-fabs(costs_[vNextWorst_]-costs_[n])/temperature))
		{
			// Swap points if necessary
