...
void PrimitiveList::reinitialise(int newSize, bool allowShrink, int maxVertices, int maxIndices, GLenum type, bool colourData, bool textureData)
{
	...
	{
		prim->initialise(maxVertices, maxIndices, type, colourData, textureData);
	}
}

// Add a new primitive to the end of the list
Primitive* PrimitiveList::addPrimitive(int maxVertices, int maxIndices, GLenum type, bool colourData, bool textureData)
{
	Primitive* newPrim = primitives_.add();
	newPrim->initialise(maxVertices, maxIndices, type, colourData, textureData);
...
// Push instance layer
void PrimitiveList::pushInstance(const QGLContext* context, GLExtensions* extensions, QGLWidget* glWidget)
{
	for (Primitive* prim = primitives_.first(); prim != NULL; prim = prim->next) prim->pushInstance(context, extensions, glWidget);
}
...
void PrimitiveList::sendToGL() const
{
