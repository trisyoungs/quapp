...
// Set text size
void TextPrimitive::setTextSize(double textSize)
{
	textSize_ = textSize;
}

// Return text size
double TextPrimitive::textSize()
{
	return textSize_;
}

// Set text
void TextPrimitive::set(QString text, TextPrimitive::TextAnchor anchorPosition)
{
...
Matrix TextPrimitive::transformationMatrix(TextFragment* fragment) const
{
	Matrix textMatrix;
	Vec3<double> lowerLeft, upperRight, anchorPos, textCentre;
	...
	anchorPos -= textCentre;

	// Construct matrix
	// -- Translate to centre of text bounding box accounting for fragment translation if one was specified
	if (fragment) textCentre -= fragment->translation();
	textMatrix.createTranslation(-textCentre * textSize_);
	textMatrix.applyScaling(textSize_, textSize_, textSize_);
	// -- Apply translation to text anchor point
	textMatrix.applyTranslation(-anchorPos * textSize_);
...
void TextPrimitive::boundingBox(Vec3<double>& lowerLeft, Vec3<double>& upperRight) const
{
...
double TextPrimitive::textWidth()
{
	Vec3<double> ll, ur;
	boundingBox(ll, ur);
	return ur.x-ll.x;
...
// Render primitive
void TextPrimitive::sendToGL() const
{
	Matrix textMatrix;
	...
	{
		textMatrix = transformationMatrix(fragment);
		glPushMatrix();
		glMultMatrixd(textMatrix.matrix());
...
int TextPrimitive::lex()
{
	...
	if (isEscape)
	{
		...
		{
			printf("Error: String '%s' is not a valid escape sequence.\n", qPrintable(token));
			return UCR_TP_FAIL;
		}
...
bool TextPrimitive::addFragment(QString text)
{
	...
	{
		printf("Internal Error: No TextFormat on stack in TextPrimitive::addFragment().\n");
		fragment->set(text);
		return false;
	}
...
bool TextPrimitive::addEscape(TextPrimitive::EscapeSequence escSeq)
{
	...
	else printf("Internal Error: No topmost TextFormat to copy from in TextPrimitive::addEscape().\n");
